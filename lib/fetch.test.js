...
test("should use custom output filename", async () => {
  ...
  try {
    ...
    assert.equal(result.filename, "custom-name.txt", "Should use custom filename");

    const files = await readdir(testDir);
    assert.ok(files.includes("custom-name.txt"), "File should exist with custom name");
  } finally {
    await rm(testDir, { recursive: true, force: true });
  }
});
...
test("should create directory if it doesn't exist", async () => {
  const testDir = join(tmpdir(), "bxl-fetch-test-mkdir-" + Date.now(), "subdir");
  ...
  try {
    ...
    assert.ok(result.path.includes(testDir), "Should create file in specified directory");
