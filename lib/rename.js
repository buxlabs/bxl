...
export async function renameFiles(input, pattern) {
  ...
  try {
    ...
    for (const file of files) {
      ...
      if (stats.isFile()) {
        const tempFile = "TEMP_PREFIX_BXL" + file;
        const tempPath = join(directory, tempFile);
        await fsRename(currentPath, tempPath);
        filesOnly.push({ file, tempFile });
      }
    }
    ...
    for (let i = 0; i < ilen; i++) {
      const { file, tempFile } = filesOnly[i];
      const filePath = join(directory, tempFile);
      ...
      if (needsDimensions) {
        ...
        if (isImage) {
          try {
            ...
            console.error(
              `✗ Failed to get dimensions for ${tempFile}:`,
              error.message
            );
            continue;
          }
      ...
      try {
        ...
        console.error(`✗ Failed to rename ${tempFile}:`, error.message);
      }
    }
